...
def sigmoid_prime(a): return a * (1 - a)  # де a = sigmoid(z)
def mse(y_pred, y_true): return np.mean((y_pred - y_true)**2)

# --- 2. Одношарова мережа ---
def train_single_layer(X_train, y_train, X_val, y_val, lr=0.5, epochs=2000, early_stop=300):
    n = X_train.shape[0]
    W = np.random.randn(1,1) * 0.5
    b = np.zeros((1,))
    history = {'train_loss':[], 'val_loss':[]}
    best_val = np.inf; wait = 0; best_params = (W.copy(), b.copy())
    for ep in range(epochs):
        ...
        if history['val_loss'][-1] < best_val - 1e-9:
            best_val = history['val_loss'][-1]; best_params = (W.copy(), b.copy()); wait = 0
        else:
            wait += 1
            if wait > early_stop:
                break
...
def train_mlp_1hidden(X_train, y_train, X_val, y_val,
                      hidden_units=8, lr=0.5, epochs=5000, early_stop=500):
    n = X_train.shape[0]
    ...
    b2 = np.zeros((1,))
    history = {'train_loss':[], 'val_loss':[]}
    best_val = np.inf; wait = 0; best_params = (W1.copy(), b1.copy(), W2.copy(), b2.copy())
    for ep in range(epochs):
        # forward
        z1 = X_train @ W1 + b1
        a1 = sigmoid(z1)
        ...
        # early stopping
        if history['val_loss'][-1] < best_val - 1e-9:
            best_val = history['val_loss'][-1]; best_params = (W1.copy(), b1.copy(), W2.copy(), b2.copy()); wait = 0
        else:
            wait += 1
            if wait > early_stop:
                break
...
# Тренування
W_s, b_s, hist_s = train_single_layer(X_train, y_train, X_val, y_val, lr=0.5, epochs=3000, early_stop=200)
W1, b1, W2, b2, hist_m = train_mlp_1hidden(X_train, y_train, X_val, y_val,
                                          hidden_units=8, lr=0.5, epochs=5000, early_stop=400)
...
plt.plot(X_all.ravel()[order], np.sin(X_all).ravel()[order], 'k-', lw=2, label='sin(x) (ціль)')
plt.plot(X_test.ravel(), y_pred_s.ravel(), 'o', label=f'Одношарова (MSE={mse_single:.4f})', alpha=0.6)
plt.plot(X_test.ravel(), y_pred_m.ravel(), 'x', label=f'MLP 8–1 (MSE={mse_mlp:.4f})', alpha=0.6)
plt.legend(); plt.title('Ціль vs Передбачення'); plt.grid(True)
...
plt.yscale('log'); plt.legend(); plt.title('Втрати – одношарова')
plt.subplot(1,2,2)
plt.plot(hist_m['train_loss'], label='train loss mlp')
plt.plot(hist_m['val_loss'],   label='val loss mlp')
plt.yscale('log'); plt.legend(); plt.title('Втрати – MLP')
...
plt.title('Активації прихованого шару (8 нейронів)'); plt.xlabel('x'); plt.legend(loc='upper right', ncol=2)
...
plt.legend(); plt.title('Гістограми ваг')
plt.subplot(1,2,2)
errors = (y_pred_m.ravel() - np.sin(X_test).ravel())
plt.hist(errors, bins=30); plt.title('Розподіл помилок (MLP)'); plt.xlabel('помилка')
