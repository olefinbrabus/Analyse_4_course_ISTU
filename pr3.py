...
service_priority = FuzzyVariable(
    "service_priority",
    (0, 1),
    {
        "low": Term("low", "trap", (0, 0, 0.2, 0.4)),
...
rules = [
    ...
    Rule(
        [
            ("budget", "high", 1.0),
            ("data_quality", "high", 1.0),
            ("product_stage", "growth", 1.0),
        ],
        "high",
        1.0,
    ),
...
fis_initial = MamdaniFIS(
    {"budget": budget, "data_quality": data_quality, "product_stage": product_stage},
    service_priority,
    rules,
)
...
true_priority = (
    0.2
    + 0.5 * (1 - np.exp(-df["budget"].values / 40))
    ...
    [
        {
            ...
            "data_quality": 0.2,
            "product_stage": 0.0,
            "label": "Idea, малий бюджет, слабкі дані",
        },
        {
            "budget": 50,
            "data_quality": 0.6,
            "product_stage": 0.5,
            "label": "MVP, середній бюджет, ок дані",
        },
        {
            "budget": 85,
            "data_quality": 0.9,
            "product_stage": 1.0,
            "label": "Growth, великий бюджет, високі дані",
        },
    ]
)
...
def rmse_current(fis, df):
    ...
    return float(
        np.sqrt(np.mean((np.array(preds) - df["target_priority"].values) ** 2))
    ), np.array(preds)
...
plt.figure()
plt.scatter(
    df["target_priority"].values,
    preds0,
    alpha=0.6,
    label=f"До оптимізації (RMSE={rmse0:.3f})",
)
plt.scatter(
    df["target_priority"].values,
    preds1,
    alpha=0.6,
    label=f"Після оптимізації (RMSE={rmse1:.3f})",
)
plt.plot([0, 1], [0, 1], "r--")
plt.xlabel("Ціль")
plt.ylabel("Прогноз FIS")
plt.legend()
plt.title("Порівняння FIS до/після оптимізації")
plt.show()
